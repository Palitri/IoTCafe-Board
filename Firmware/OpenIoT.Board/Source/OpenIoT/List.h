...
public:
	...
	// Creates a list with the capacity to hold the given number of elements
	List(int capacity = 64)
	{
		this->capacity = 0;
		this->count = 0;
		this->values = 0;
	...
	void Copy(T* values, int numValues)
	{
		this->Copy(0, values, 0, numValues);
	}

	void Copy(int offset, T* values, int valuesOffset, int numValues)
	{
		this->SetSize(offset + numValues);
	...
	void Move(int originalIndex, int newIndex, int count)
	{
		...
		T* temp = new T[count];
		Mem::Copy(temp, &this->values[originalIndex], count * sizeof(T));
		...
		Mem::Copy(&this->values[newIndex], temp, count * sizeof(T));
		delete[] temp;
	};
	...
	bool Remove(T value)
	{
		...
		if (index != List::ListIndex_NotFound)
		{
			this->RemoveAt(index);
			return true;
		}
	...
	int RemoveAll(T value)
	{
		int numRemoved = 0;
		int index = this->IndexOf(value);
		while (index != List::ListIndex_NotFound)
		{
			this->RemoveAt(index);
			numRemoved++;
	...
	// Returns the index of the first encountered item of the given value. If such item is not present, the return value is -AxList::ListIndex_NotFound
	int IndexOf(T value)
	{
		for (int i = 0; i < this->count; i++)
			if (Mem::Compare(&this->values[i], &value, sizeof(T)) == 0)
				return i;

		return List::ListIndex_NotFound;
	};

	// Returns the index of the first encountered item of the given value, starting the search from the end of the list to the beginning. If such item is not present, the return value is -AxList::ListIndex_NotFound
